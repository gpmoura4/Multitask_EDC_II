...
# 🔹 Clients globais (podem ser None se a env não estiver setada)
...
def get_client_for_model(model_name: str):
    """
    Dado o nome de um modelo, retorna (client, provider):

      - provider == "openai" → usar openai_client
      - provider == "groq"   → usar groq_client
      - provider == "local"  → modelo pequeno / Hugging Face local (sem client remoto)

    Exemplo:
        client, provider = get_client_for_model("gpt-4o")
        client, provider = get_client_for_model("llama-3.1-70b-versatile")
        client, provider = get_client_for_model("mistral-7b-instruct")
    """
    ...
    if any(prefix in name_lower for prefix in ["gpt-3", "gpt-4", "o1", "o3"]):
        if openai_client is None:
            raise ValueError(
                "OPENAI_API_KEY não definida, mas um modelo GPT foi solicitado."
            )
        return openai_client, "openai"

    # Modelos LLaMA ou que contenham 'groq' → Groq
    if ("llama" in name_lower) or ("groq" in name_lower):
        if groq_client is None:
            raise ValueError(
                "GROQ_API_KEY não definida, mas um modelo LLaMA/Groq foi solicitado."
            )
        return groq_client, "groq"

    # Qualquer outro nome → trata como modelo local (HF / pequeno)
    return None, "local"


def get_sni_json(task_name):
    url = "https://raw.githubusercontent.com/allenai/natural-instructions/master/tasks/<task>.json".replace("<task>",
                                                                                                            task_name)
...
def reconstruct_instruction(data, num, full=True, tf=False):
    ...
    if full:
        ...
        filtered_contexts = '\n'.join(
            [f"{v}" for i in range(1, num + 1) for k, v in contexts.items() if k.startswith(str(i))])
        final_string = query + "\n" + filtered_instructions + "\n\n" + filtered_contexts
    else:
        if str(num) not in instructions:
            raise KeyError(f"Instrução {num} não encontrada. Instruções disponíveis: {list(instructions.keys())}")
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
                ...
                for idx, gpt_input in gpt_input_dict[len(answers):]:
                    # aqui assumimos que 'idx' é o nome do modelo (ajuste conforme sua estrutura real)
...
def get_answer(instance, model):  # model: nome do modelo, ex: "gpt-4o", "llama-3.1-70b-versatile"
    """
    Faz uma chamada ao modelo remoto adequado (OpenAI ou Groq),
    com base no nome do modelo passado em `model`.
    """
    client, provider = get_client_for_model(model)

    if provider == "local" or client is None:
        raise ValueError(
            f"get_answer só suporta modelos remotos (OpenAI ou Groq). "
            f"Modelo recebido: {model}"
        )
